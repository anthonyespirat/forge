...
Agent definitions live in `agents/` as Markdown files with YAML frontmatter (`name`, `description`, `tools`, `dispatch`).

**Registration:**

- **Claude Code**: Agents are symlinked into `.claude/agents/` so the `Agent` tool discovers them by name.
- **OpenCode**: The `forge-plugin.js` reads `agents/` at startup and registers each as a subagent in `config.agent`, making them available as `subagent_type` values in the `Task` tool.
